...
4. **Establish Data Plane Linkage:** Navigate directly to the newly provisioned SharePoint Online (SPO) site collection URL interface (`https://[your-tenant-name].sharepoint.com/sites/Acquisition-Vault-SPO`). Click the settings gear icon in the top right corner and select **Site permissions** to slide out the modern access management control panel. Click the **Share site** button at the top of this panel, type `Human Auditor User` into the target identity field, select the user from the directory pop-up list, and change the permission dropdown assignment from *Edit* directly to **Read**. Clear the email notification transmission checkbox, and click **Save** to map the user object to the site and enforce strict read-only access to the document library plane.
